...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const vehicleRequestsSection = document.getElementById("vehicleRequestsSection");
  ...
  // Solo el administrador revisa solicitudes vehiculares
if (usuarioActivo.rol === "Administrador" && vehicleRequestsSection) {
  vehicleRequestsSection.classList.remove("hidden");
  cargarSolicitudesVehiculares();
}
...
// ===============================
// REVISIÓN DE SOLICITUDES VEHICULARES
// ===============================

function cargarSolicitudesVehiculares() {
  const vehicleRequestsTableBody = document.getElementById("vehicleRequestsTableBody");

  if (!vehicleRequestsTableBody) {
    return;
  }

  const solicitudes = JSON.parse(localStorage.getItem("solicitudesVehiculo")) || [];

  vehicleRequestsTableBody.innerHTML = "";

  if (solicitudes.length === 0) {
    vehicleRequestsTableBody.innerHTML = `
      <tr>
        <td colspan="13">No hay solicitudes vehiculares registradas.</td>
      </tr>
    `;
    return;
  }
  ...
  solicitudes.forEach(function(solicitud) {
    ...
    const fotoVehiculo = solicitud.fotoVehiculo || generarImagenPlaceholder("V");
    const fotoPlaca = solicitud.fotoPlaca || generarImagenPlaceholder("P");

    const claseEstado = obtenerClaseSolicitudVehicular(solicitud.estadoSolicitud);

    let accionHTML = "Sin acciones";

    if (solicitud.estadoSolicitud === "Pendiente") {
      accionHTML = `
        <button class="mark-read-btn" onclick="aprobarSolicitudVehicular(${solicitud.id})">
          Aprobar
        </button>

        <button class="delete-btn" onclick="rechazarSolicitudVehicular(${solicitud.id})">
          Rechazar
        </button>
      `;
    }
    ...
    fila.innerHTML = `
      ...
      <td>${solicitud.dniAlumno || "-"}</td>
      <td>${solicitud.areaAlumno || "-"}</td>
      ...
      <td>
        <img src="${fotoPlaca}" class="plate-photo" alt="Foto placa">
      </td>
      <td>
        <span class="status-badge ${claseEstado}">
    ...
    vehicleRequestsTableBody.appendChild(fila);
  });
}

function aprobarSolicitudVehicular(id) {
  ...
  const placaExiste = vehiculos.some(function(vehiculo) {
    return String(vehiculo.placa || "").toUpperCase() === String(solicitud.placa || "").toUpperCase();
  });

  if (placaExiste) {
    alert("No se puede aprobar. Ya existe un vehículo registrado con esa placa.");
    return;
  }

  const confirmar = confirm("¿Deseas aprobar esta solicitud y registrar el vehículo como autorizado?");
  ...
  const nuevoVehiculo = {
    id: Date.now(),
    dniPropietario: solicitud.dniAlumno,
    nombrePropietario: solicitud.nombreAlumno,
    areaPropietario: solicitud.areaAlumno,
    placa: solicitud.placa,
    marca: solicitud.marca,
    modelo: solicitud.modelo,
    color: solicitud.color,
    tipoVehiculo: solicitud.tipo,
    estadoVehiculo: "Autorizado",
    observacionesVehiculo: solicitud.observacion || "Solicitud aprobada por administración.",
  ...
  solicitudes = solicitudes.map(function(item) {
    if (item.id === id) {
      ...
      item.fechaRevision = new Date().toLocaleString();
    }
  ...
  mostrarMensajeVehiculo("Solicitud aprobada y vehículo registrado correctamente.", "success");
}

function rechazarSolicitudVehicular(id) {
  let solicitudes = JSON.parse(localStorage.getItem("solicitudesVehiculo")) || [];
  ...
  solicitudes = solicitudes.map(function(item) {
    if (item.id === id) {
      ...
      item.motivoRechazo = motivo.trim() || "Solicitud rechazada por administración.";
      item.fechaRevision = new Date().toLocaleString();
    }
  ...
  mostrarMensajeVehiculo("Solicitud rechazada correctamente.", "success");
}

function obtenerClaseSolicitudVehicular(estado) {
  if (estado === "Aprobada") {
    return "status-autorizado";
  }
